...
//    return [_cache objectForKey:_URL];
    NSString* documentsDirectory = [[AsyncImageLoader imageDirectory] stringByAppendingString:[[_URL absoluteString] MD5String]];
    return [UIImage imageWithContentsOfFile:[documentsDirectory stringByAppendingString:@".png"]];
}

- (BOOL)isInCache
{
    NSString* filePath = [[AsyncImageLoader imageDirectory] stringByAppendingString:[[_URL absoluteString] MD5String]];
    return [[NSFileManager defaultManager] fileExistsAtPath:[filePath stringByAppendingString:@".png"]];
...
- (void)cacheImage:(UIImage *)image
{
	if (!_cancelled)
	{
        if (image && _URL)
        {
            ...
            if (storeInCache)
            {
               // NSLog(@"URL IS:%@", [_URL absoluteString]);
                [self saveImage:image withFileName:[_URL absoluteString]];
...
//                                  regularExpressionWithPattern:@"(MSP[^/])(.+)([?])"
//                                  options:0
//                                  error:&error];
//    
...
//    
//    NSString *result = [fileName substringWithRange:range];
//    return result;
//}
...
- (void)saveImage: (UIImage*)image withFileName:(NSString*) fileName
{
    if (image != nil)
    {
        ...
        NSString* documentsDirectory = [AsyncImageLoader imageDirectory];
        NSError* error = nil;
        ...
        if(![[NSFileManager defaultManager] createDirectoryAtPath:documentsDirectory
                                  withIntermediateDirectories:NO
                                                   attributes:nil
                                                            error:&error]){}
        NSString* path = [[documentsDirectory stringByAppendingString:[fileName MD5String]] stringByAppendingString:@".png"];
        NSData* data = UIImagePNGRepresentation(image);
      //  NSLog(@"saved image location is:%@", path);
...
//    UIImage *image = [_cache objectForKey:URL];
    ...
    UIImage* image = [AsyncImageLoader loadImage:path];
    if (image)
    {
        ...
        //DDLogVerbose(@"Image is in directory");
        return;
    }
    //DDLogVerbose(@"Image was not in directory");
...
+ (UIImage*)loadImage:(NSString*)fileName
{
    ...
    NSString* filePath = [[[AsyncImageLoader imageDirectory] stringByAppendingString:[fileName MD5String]] stringByAppendingString:@".png"];
...
+ (NSString*) imageDirectory
{
    ...
    NSString *imageDir = [paths objectAtIndex:0];
    imageDir = [imageDir stringByAppendingString:@"/wolframImages/"];
    return imageDir;
}
